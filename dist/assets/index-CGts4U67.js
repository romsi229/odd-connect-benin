(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const a of s)if(a.type==="childList")for(const o of a.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&n(o)}).observe(document,{childList:!0,subtree:!0});function i(s){const a={};return s.integrity&&(a.integrity=s.integrity),s.referrerPolicy&&(a.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?a.credentials="include":s.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function n(s){if(s.ep)return;s.ep=!0;const a=i(s);fetch(s.href,a)}})();function f(t,e,i){return["auth"].includes(window.currentPage)?`
  ...
  <div class="fade-up">
    ...
    <div class="modules-grid mb-24">
      ${[{icon:"📊",name:"Dashboard ODD",desc:"Suivi des 17 indicateurs en temps réel",page:"dashboard",bg:"#E1F5EE",ic:"#0F6E56"},{icon:"📢",name:"Plaidoyer",desc:"Campagnes et pétitions nationales",page:"plaidoyer",bg:"#FFF8E1",ic:"#E8A000"},{icon:"🔔",name:"Veille citoyenne",desc:"Signalements géolocalisés",page:"veille",bg:"#FFF0EE",ic:"#D85A30"},{icon:"🗺️",name:"Cartographie",desc:"Carte interactive du Bénin",page:"carte",bg:"#EAF4FF",ic:"#185FA5"},{icon:"🤖",name:"IA & Data",desc:"Analyses et recommandations",page:"dashboard",bg:"#F0EEFF",ic:"#6C63CC"},{icon:"💬",name:"Forum",desc:"Discussions communautaires",page:"forum",bg:"#E1F5EE",ic:"#0F6E56"},{icon:"📚",name:"Bibliothèque",desc:"Rapports et ressources",page:"bibliotheque",bg:"#FFE9F3",ic:"#C94F7B"},{icon:"📅",name:"Calendrier",desc:"Agenda des événements ODD",page:"calendrier",bg:"#F5F5F5",ic:"#4A6960"},{icon:"🌍",name:"Partenaires",desc:"Réseau d'organisations",page:"home",bg:"#E1F5EE",ic:"#0A4D3C"}].map(e=>`
        <div class="module-card" onclick="navigate('${e.page}')">
          <div class="module-icon" style="background:${e.bg};">${e.icon}</div>
          <div class="module-name">${e.name}</div>
          <div class="module-desc">${e.desc}</div>
        </div>
      `).join("")}
    </div>
  </div>`}function k(){const t=c.map(a=>`
  ...
  `).join(""),s=c.filter(a=>a.pct<60).sort((a,o)=>a.pct-o.pct).slice(0,5).map(a=>{const o=a.pct>=40?"#E8A000":"#D85A30";return`
    <div class="prog-item">
      <div class="prog-top">
  ...
  <div class="fade-up">
    ...
    <div class="campaign-grid">${m.map(e=>{const i=Math.round(e.signatures/e.goal*100);return`
    <div class="campaign-card">
      <div class="camp-tag" style="background:${e.bg};color:${e.color}">${e.oddLabel} · ${e.tags[0]}</div>
      <div class="camp-title">${e.title}</div>
      <div class="camp-desc">${e.desc}</div>
      <div class="camp-progress-track">
        <div class="camp-progress-fill" style="width:${i}%;background:${e.color}"></div>
      </div>
      <div class="camp-meta">
        <span><strong>${e.signatures.toLocaleString("fr")}</strong> signatures sur ${e.goal.toLocaleString("fr")}</span>
        <span>⏱ ${e.days} jours restants</span>
      </div>
      <div class="camp-actions">
        <button class="btn btn-primary btn-sm" style="background:${e.color}" onclick="navigate('campagne-detail')">✍️ Signer</button>
  ...
  <div class="fade-up">
    ...
    <div class="grid-3 mb-16">
      <div>
        ...
        <div class="signal-list">${d.map(e=>`
    <div class="signal-card">
      <div class="signal-sev" style="background:${e.severity==="high"?"#D85A30":e.severity==="medium"?"#E8A000":"#1D9E75"}"></div>
      <div style="flex:1">
        <div class="signal-odd-tag" style="background:${e.oddColor}22;color:${e.oddColor}">${e.oddLabel}</div>
        <div class="signal-title">${e.title}</div>
        <div class="signal-desc">${e.desc}</div>
        <div class="signal-footer">
          <span class="signal-loc">📍 ${e.location} · ${e.time}</span>
          <span class="signal-status" style="background:${e.statusColor}22;color:${e.statusColor}">${e.status}</span>
        </div>
      </div>
    </div>
  `).join("")}</div>
      ...
      <div style="display:flex;flex-direction:column;gap:12px;">
        ...
        <div class="card">
          ...
          <div class="card-body">
            ${y.map((t,e)=>`
              <div class="commune-row">
                <span class="commune-rank">${e+1}</span>
                <span class="commune-name">${t.name}</span>
                <div class="commune-track"><div class="commune-fill" style="width:${t.pct}%"></div></div>
                <span class="commune-pct">${t.pct}%</span>
              </div>`).join("")}
          </div>
        </div>
      </div>
  ...
  <div class="fade-up">
    ...
    <div class="grid-3">
      <div class="forum-list">${u.map(e=>`
    <div class="forum-card">
      <div class="forum-tags">
        ${e.tags.map(i=>`<span class="forum-tag" style="background:var(--green-soft);color:var(--green-mid)">${i}</span>`).join("")}
      </div>
      <div class="forum-title">${e.title}</div>
      <div class="forum-excerpt">${e.excerpt}</div>
      <div class="forum-footer">
        <span>👤 ${e.author}</span>
        <span>⏱ ${e.time}</span>
        <span>💬 ${e.replies} réponses</span>
  ...
  `).join("")}</div>
      <div style="display:flex;flex-direction:column;gap:12px;">
        <div class="card">
          ...
          <div class="card-body">
            ${[{name:"Mariame T.",posts:203,badge:"🏆"},{name:"Ibrahim M.",posts:127,badge:"⭐"},{name:"Reine F.",posts:89,badge:"🌱"},{name:"Kouassi A.",posts:41,badge:"💬"}].map(e=>`
              <div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--gray-100);">
                <div style="width:28px;height:28px;border-radius:50%;background:var(--green-soft);display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:600;color:var(--green-mid);">${e.name[0]}</div>
                <div style="flex:1;"><div style="font-size:12px;font-weight:600">${e.name}</div><div style="font-size:11px;color:var(--text-light)">${e.posts} contributions</div></div>
                <span>${e.badge}</span>
              </div>`).join("")}
          </div>
        </div>
        <div class="card">
          ...
          <div class="card-body" style="display:flex;flex-wrap:wrap;gap:6px;">
            ${["#EauPotable","#ÉnergiesSolaires","#Reboisement","#ÉgalitéFemmes","#DéchetsPlastiques","#ClimatBénin","#JeunesODD"].map(e=>`<span style="background:var(--green-soft);color:var(--green-mid);border-radius:20px;padding:4px 10px;font-size:12px;cursor:pointer">${e}</span>`).join("")}
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="fade-up">
    ...
    <div class="doc-grid">${g.map(e=>`
    <div class="doc-card">
      <div class="doc-type-icon">${e.icon}</div>
      <div class="doc-title">${e.title}</div>
      <div class="doc-desc">${e.desc}</div>
      <div class="doc-footer">
        <span style="background:var(--green-soft);color:var(--green-mid);border-radius:4px;padding:2px 8px;font-size:11px;font-weight:600">${e.type}</span>
        <span>${e.date} · ${e.pages}p</span>
      </div>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:4px;text-align:center;font-size:11px;">
            ${["L","M","M","J","V","S","D"].map(e=>`<div style="font-weight:600;color:var(--text-light);padding:4px">${e}</div>`).join("")}
            ${Array.from({length:30},(e,i)=>{const n=i+1;return`<div style="padding:6px 4px;border-radius:4px;cursor:pointer;${[15,22].includes(n)?"background:var(--green-soft);color:var(--green-mid);font-weight:600":""}">${n}</div>`}).join("")}
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="grid-3">
      <div style="display:flex;flex-direction:column;gap:14px;">
        ...
        <div class="card">
          ...
          <div class="card-body" style="display:flex;flex-wrap:wrap;gap:8px;">
            ${["🏆 Top contributeur","🌱 Reboiseur","💧 Défenseur eau","📊 Data citoyen","🎤 Plaideur"].map(t=>`<span style="background:var(--gray-100);border-radius:20px;padding:4px 10px;font-size:12px;">${t}</span>`).join("")}
          </div>
        </div>
      </div>
      ...
      <div style="display:flex;flex-direction:column;gap:14px;">
        ...
        <div class="card">
          ...
          <div class="card-body" style="display:flex;flex-wrap:wrap;gap:6px;">
            ${[6,7,13,15].map(t=>{const e=c.find(n=>n.n===t);return`<div style="background:${e.pct>=70?"#1D9E75":e.pct>=40?"#BA7517":"#A32D2D"};border-radius:6px;padding:6px 10px;color:white;font-size:12px;font-weight:700;">ODD ${t}</div>`}).join("")}
          </div>
        </div>
        ...
        <div class="card">
          <div class="card-body" style="padding:24px;">
            <div style="display:flex;gap:8px;margin-bottom:14px;">
              ${t.tags.map(s=>`<span style="background:${t.bg};color:${t.color};border-radius:20px;padding:4px 12px;font-size:12px;font-weight:600">${s}</span>`).join("")}
              <span style="background:#E1F5EE;color:#0F6E56;border-radius:20px;padding:4px 12px;font-size:12px;font-weight:600">🟢 Active</span>
            </div>
            <h2 style="font-family:var(--font-display);font-size:24px;font-weight:700;color:var(--green-deep);margin-bottom:10px;">${t.title}</h2>
            ...
            <div style="margin-bottom:8px;display:flex;justify-content:space-between;align-items:baseline;">
              <span style="font-family:var(--font-display);font-size:28px;font-weight:700;color:${t.color}">${t.signatures.toLocaleString("fr")}</span>
              <span style="font-size:13px;color:var(--text-light)">sur ${t.goal.toLocaleString("fr")} objectif</span>
            </div>
            <div style="height:10px;background:var(--gray-100);border-radius:5px;margin-bottom:10px;">
              <div style="height:10px;border-radius:5px;background:${t.color};width:${e}%;transition:width .6s"></div>
            </div>
            <div style="display:flex;justify-content:space-between;font-size:12px;color:var(--text-light);margin-bottom:20px;">
              <span><strong style="color:var(--text-main)">${e}%</strong> de l'objectif</span>
              <span>⏱ <strong style="color:var(--text-main)">${t.days} jours</strong> restants</span>
            </div>
        ...
        <div class="card-body" style="padding:28px;display:flex;flex-direction:column;gap:20px;">
          ...
          <div>
            ...
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;" id="odd-selector">
              ${[{n:6,label:"💧 Eau propre",color:"#26BDE2"},{n:7,label:"⚡ Énergie",color:"#FCC30B"},{n:11,label:"🏘️ Villes",color:"#FD9D24"},{n:13,label:"🌡️ Climat",color:"#3F7E44"},{n:14,label:"🐟 Mer",color:"#0A97D9"},{n:15,label:"🌿 Forêts",color:"#56C02B"},{n:3,label:"❤️ Santé",color:"#4C9F38"},{n:12,label:"♻️ Déchets",color:"#BF8B2E"}].map(t=>`
                <button onclick="selectODD(this,'${t.color}')"
          ...
          <div>
            ...
            <div style="display:flex;gap:10px;">
              ${[{val:"low",label:"🟢 Faible",desc:"Peut attendre"},{val:"medium",label:"🟡 Modéré",desc:"À traiter sous 48h"},{val:"high",label:"🔴 Urgent",desc:"Intervention rapide nécessaire"}].map(t=>`
                <label style="flex:1;border:1px solid var(--gray-200);border-radius:var(--radius-sm);padding:10px;cursor:pointer;text-align:center;">
                  <input type="radio" name="urgence" value="${t.val}" style="display:none">
    ...
    <div class="grid-3 mb-16">
      <div style="display:flex;flex-direction:column;gap:14px;">
        ...
        <div class="card">
          ...
          <div class="card-body">
            ${[{label:"% population accès eau potable",val:"52%",trend:"▲ +3%",color:"#1D9E75"},{label:"% ménages eau courante domicile",val:"24%",trend:"▲ +1.5%",color:"#1D9E75"},{label:"Délai moyen accès point d'eau",val:"38 min",trend:"▼ -5 min",color:"#1D9E75"},{label:"Communes avec eau traitée",val:"28 / 77",trend:"▲ +4",color:"#1D9E75"},{label:"Budget gouvernemental alloué",val:"12.4 Md FCFA",trend:"▲ +8%",color:"#1D9E75"},{label:"Puits fonctionnels recensés",val:"4 820",trend:"▼ -120",color:"#E8A000"}].map(e=>`
              <div style="display:flex;align-items:center;justify-content:space-between;padding:9px 0;border-bottom:1px solid var(--gray-100);">
                <span style="font-size:12.5px;color:var(--text-sub);flex:1;padding-right:12px">${e.label}</span>
                <div style="text-align:right">
                  <div style="font-size:14px;font-weight:700;color:var(--green-deep)">${e.val}</div>
                  <div style="font-size:11px;color:${e.color}">${e.trend}</div>
                </div>
              </div>`).join("")}
          </div>
        </div>
      ...
      <div style="display:flex;flex-direction:column;gap:14px;">
        ...
        <div class="card">
          ...
          <div class="card-body">
            ${[{dep:"Littoral",val:89},{dep:"Atlantique",val:71},{dep:"Ouémé",val:64},{dep:"Borgou",val:48},{dep:"Zou",val:44},{dep:"Atacora",val:31},{dep:"Alibori",val:28},{dep:"Donga",val:35}].map(e=>`
              <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
                <span style="font-size:12px;color:var(--text-main);width:80px;flex-shrink:0">${e.dep}</span>
                <div style="flex:1;height:6px;background:var(--gray-100);border-radius:3px">
                  <div style="height:6px;border-radius:3px;background:${e.val>=60?"#1D9E75":e.val>=40?"#E8A000":"#D85A30"};width:${e.val}%"></div>
                </div>
                <span style="font-size:12px;font-weight:600;width:32px;text-align:right">${e.val}%</span>
              </div>`).join("")}
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div class="card-body">
            ${m.filter(e=>e.odd===6||e.odd===14).map(e=>`
              <div onclick="navigate('campagne-detail')" style="padding:10px;border:1px solid var(--gray-200);border-radius:var(--radius-sm);cursor:pointer;margin-bottom:8px;background:white;transition:all .2s"
                ...
                <div style="font-size:12.5px;font-weight:600;margin-bottom:3px">${e.title}</div>
                <div style="font-size:11px;color:var(--text-light)">${e.signatures.toLocaleString("fr")} signatures · ${e.days} jours restants</div>
              </div>`).join("")}
  ...
  <div class="fade-up">
    ...
    <div class="card">
      <div class="card-body" style="padding:0;">
        ${t.map((e,i)=>`
          <div style="display:flex;align-items:flex-start;gap:14px;padding:16px 20px;border-bottom:${i<t.length-1?"1px solid var(--gray-100)":"none"};background:${e.unread?"var(--green-pale)":"white"};cursor:pointer;transition:background .2s"
            onmouseover="this.style.background='${e.unread?"#e0f5ec":"var(--gray-100)"}'" onmouseout="this.style.background='${e.unread?"var(--green-pale)":"white"}'">
            <div style="width:40px;height:40px;border-radius:10px;background:${e.color}22;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">${e.icon}</div>
            <div style="flex:1">
              <div style="display:flex;align-items:center;gap:6px;margin-bottom:2px">
                <span style="font-size:11px;font-weight:600;color:${e.color};background:${e.color}18;border-radius:4px;padding:1px 7px">${e.type}</span>
                ${e.unread?'<span style="width:7px;height:7px;border-radius:50%;background:var(--green-live);display:inline-block"></span>':""}
              </div>
              <div style="font-size:13.5px;font-weight:${e.unread?"600":"400"};color:var(--text-main);margin-bottom:2px">${e.title}</div>
              <div style="font-size:12px;color:var(--text-sub)">${e.sub}</div>
            </div>
            <div style="font-size:11px;color:var(--text-light);flex-shrink:0;margin-top:2px">${e.time}</div>
          </div>`).join("")}
      </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:220px 1fr;gap:20px;">
      ...
      <div class="card" style="height:fit-content;">
        <div class="card-body" style="padding:10px;">
          ${[{id:"compte",label:"👤 Compte & Profil"},{id:"notifs",label:"🔔 Notifications"},{id:"confidentialite",label:"🔒 Confidentialité"},{id:"langue",label:"🌍 Langue & Région"},{id:"apparence",label:"🎨 Apparence"}].map((e,i)=>`
            <button onclick="showSection('${e.id}')" id="sec-btn-${e.id}"
      ...
      <div>
        ...
        <div id="sec-compte" class="card">
          ...
          <div class="card-body" style="padding:24px;display:flex;flex-direction:column;gap:18px;">
            ...
            ${[{label:"Prénom",val:"Yao",type:"text"},{label:"Nom",val:"Kouassi",type:"text"},{label:"Email",val:"yao.kouassi@email.com",type:"email"},{label:"Commune",val:"Cotonou",type:"text"},{label:"Téléphone",val:"+229 97 XX XX XX",type:"tel"}].map(e=>`
              <div>
                <label style="display:block;font-size:12px;font-weight:600;color:var(--text-sub);margin-bottom:6px">${e.label}</label>
                <input type="${e.type}" value="${e.val}"
                  style="width:100%;padding:10px 14px;border:1px solid var(--gray-200);border-radius:var(--radius-sm);font-size:14px;font-family:var(--font-body);outline:none;color:var(--text-main)">
              </div>`).join("")}
            <div style="display:flex;gap:10px;">
        ...
        <div id="sec-notifs" class="card" style="display:none;">
          ...
          <div class="card-body" style="padding:24px;">
            ${[{label:"Nouvelles campagnes de plaidoyer",detail:"Quand une campagne est lancée dans ta commune"},{label:"Signalements dans ta zone",detail:"Alertes géolocalisées près de chez toi"},{label:"Réponses à tes posts forum",detail:"Quand quelqu'un répond à ta discussion"},{label:"Mises à jour des pétitions signées",detail:"Progression et résultats de tes pétitions"},{label:"Événements à venir",detail:"Rappels 24h avant les événements inscrits"},{label:"Nouveaux documents ODD",detail:"Quand un rapport ou guide est publié"},{label:"Newsletter mensuelle",detail:"Résumé mensuel des activités ODD Bénin"}].map((e,i)=>`
              <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 0;border-bottom:${i<6?"1px solid var(--gray-100)":"none"}">
                <div>
                  <div style="font-size:13.5px;font-weight:500;margin-bottom:2px">${e.label}</div>
                  <div style="font-size:12px;color:var(--text-light)">${e.detail}</div>
                </div>
          ...
          <div class="card-body" style="padding:24px;display:flex;flex-direction:column;gap:16px;">
            <div style="padding:16px;border:1px solid var(--gray-200);border-radius:var(--radius-md);">
              ...
              <div style="display:flex;flex-direction:column;gap:8px;">
                ${[{v:"public",l:"🌍 Public — Visible par tous"},{v:"members",l:"👥 Membres — Visible par les inscrits"},{v:"private",l:"🔒 Privé — Visible uniquement par toi"}].map((e,i)=>`
                  <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px;">
                    <input type="radio" name="visibility" value="${e.v}" ${i===0?"checked":""} style="accent-color:var(--green-live)"> ${e.l}
                  </label>`).join("")}
              </div>
            </div>
            <div style="padding:16px;border:1px solid var(--gray-200);border-radius:var(--radius-md);">
          ...
          <div class="card-body" style="padding:24px;display:flex;flex-direction:column;gap:20px;">
            <div>
              ...
              <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;">
                ${[{id:"light",label:"☀️ Clair",active:!0},{id:"dark",label:"🌙 Sombre",active:!1},{id:"auto",label:"💻 Système",active:!1}].map(e=>`
                  <button style="padding:16px 12px;border:${e.active?"2px solid var(--green-live)":"1px solid var(--gray-200)"};border-radius:var(--radius-md);font-size:13px;font-weight:${e.active?"600":"400"};cursor:pointer;background:${e.active?"var(--green-pale)":"white"};color:${e.active?"var(--green-mid)":"var(--text-main)"}">
                    ${e.label}
                  </button>`).join("")}
              </div>
            </div>
            ...
            <div>
              ...
              <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
                ${[{l:"Compact",s:!0},{l:"Confortable",s:!1}].map(e=>`
                  <button style="padding:12px;border:${e.s?"2px solid var(--green-live)":"1px solid var(--gray-200)"};border-radius:var(--radius-md);font-size:13px;cursor:pointer;background:${e.s?"var(--green-pale)":"white"};color:${e.s?"var(--green-mid)":"var(--text-main)"}">${e.l}</button>`).join("")}
              </div>
            </div>
          </div>
        </div>
  ...
  <div class="fade-up">
    ...
    <div class="grid-cols-3">
      ${t.map(i=>{const n=e[i.type]||"#888";return`
    ...
    <div class="grid-3 mb-24">
      <div style="display:flex;flex-direction:column;gap:16px;">
        ...
        <div class="card">
          ...
          <div class="card-body">
            ${[{icon:"🌍",v:"Inclusivité",d:"Accessible à tous les béninois, sans exclusion"},{icon:"📊",v:"Transparence",d:"Données ouvertes et processus vérifiables"},{icon:"🤝",v:"Collaboration",d:"Co-construction avec les communautés"},{icon:"⚡",v:"Action",d:"Du plaidoyer aux résultats concrets"}].map(e=>`
              <div style="display:flex;gap:10px;padding:8px 0;border-bottom:1px solid var(--gray-100);">
                <span style="font-size:18px">${e.icon}</span>
                <div><div style="font-size:13px;font-weight:600">${e.v}</div><div style="font-size:12px;color:var(--text-sub)">${e.d}</div></div>
              </div>`).join("")}
          </div>
        </div>
      </div>
      ...
      <div style="display:flex;flex-direction:column;gap:16px;">
          ...
          <div class="card-body">
            ${t.map(e=>`
              <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--gray-100);">
                <div style="width:40px;height:40px;border-radius:50%;background:${e.bg};color:${e.color};font-size:13px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0">${e.init}</div>
                <div><div style="font-size:13px;font-weight:600">${e.name}</div><div style="font-size:12px;color:var(--text-sub)">${e.role}</div></div>
              </div>`).join("")}
        ...
        <div class="card">
          ...
          <div class="card-body">
            ${[{icon:"📧",label:"Email",val:"contact@oddconnect.bj"},{icon:"📱",label:"WhatsApp",val:"+229 01 XX XX XX"},{icon:"🌐",label:"Site",val:"www.oddconnect.bj"},{icon:"🐦",label:"Twitter / X",val:"@ODDConnectBenin"}].map(e=>`
              <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--gray-100);">
                <span style="font-size:16px;width:24px;text-align:center">${e.icon}</span>
                <div><div style="font-size:11px;color:var(--text-light)">${e.label}</div><div style="font-size:13px;color:var(--green-mid);font-weight:500">${e.val}</div></div>
              </div>`).join("")}
          </div>
        </div>
      </div>
  ...
  </div>`}function q(){const t=[{icon:"👥",label:"Utilisateurs",val:"12 418",sub:"34 nouveaux aujourd'hui",color:"#1D9E75"},{icon:"🚨",label:"Signalements à valider",val:"23",sub:"Attente modération",color:"#D85A30"},{icon:"📢",label:"Campagnes actives",val:"34",sub:"8 en attente d'approbation",color:"#E8A000"},{icon:"📄",label:"Documents à valider",val:"7",sub:"Soumis cette semaine",color:"#185FA5"}],e=[{time:"14:32",event:"Nouveau signalement soumis",user:"Amina K.",type:"signal",color:"#D85A30"},{time:"14:18",event:"Campagne validée et publiée",user:"Admin",type:"admin",color:"#1D9E75"},{time:"13:55",event:"Inscription nouvel utilisateur",user:"Ibrahim M.",type:"user",color:"#185FA5"},{time:"13:40",event:"Document mis en ligne",user:"INSAE Bénin",type:"doc",color:"#6C63CC"},{time:"12:18",event:"Signalement marqué résolu",user:"Mairie Abomey",type:"signal",color:"#1D9E75"},{time:"11:44",event:"Campagne signalée comme spam",user:"Kofi T.",type:"flag",color:"#E8A000"}];return`
  <div class="fade-up">
  ...
  <div class="fade-up">
    ...
    <div id="results-list" style="display:flex;flex-direction:column;gap:8px;">
      ${[{type:"Campagne",icon:"📢",color:"#E8A000",bg:"#FFF8E1",title:"Eau pour tous au Nord-Bénin",sub:"4 820 signatures · ODD 6 · Active",page:"campagne-detail"},{type:"ODD",icon:"💧",color:"#26BDE2",bg:"#EAF8FD",title:"ODD 6 — Eau propre et assainissement",sub:"Score Bénin : 48% · Modéré",page:"odd-detail"},{type:"Signalement",icon:"🚨",color:"#D85A30",bg:"#FFF0EE",title:"Déversement industriel dans l'Ouémé",sub:"Adjohoun · Il y a 2h · En cours",page:"signal-detail"},{type:"Forum",icon:"💬",color:"#6C63CC",bg:"#F0EEFF",title:"Comment impliquer les jeunes dans le suivi des ODD ?",sub:"24 réponses · 41 likes · Kouassi A.",page:"forum-detail"},{type:"Document",icon:"📄",color:"#1D9E75",bg:"#E1F5EE",title:"Rapport ODD Bénin 2024",sub:"INSAE · Mars 2024 · 128 pages",page:"doc-detail"},{type:"Événement",icon:"📅",color:"#185FA5",bg:"#EAF4FF",title:"Forum National ODD Bénin 2025",sub:"15 Juin · Cotonou, Palais des Congrès",page:"event-detail"}].map(e=>`
        <div class="search-result-card" data-type="${e.type}" onclick="navigate('${e.page}')"
          ...
          <div style="width:42px;height:42px;border-radius:10px;background:${e.bg};display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0">${e.icon}</div>
          <div style="flex:1">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:3px">
              <span style="font-size:10px;font-weight:600;background:${e.bg};color:${e.color};border-radius:4px;padding:2px 7px">${e.type}</span>
            </div>
            <div style="font-size:13.5px;font-weight:600;color:var(--text-main);margin-bottom:2px">${e.title}</div>
            <div style="font-size:12px;color:var(--text-sub)">${e.sub}</div>
          </div>
  ...
  <\/script>`}function G(){const t=u[0],e=[{init:"RM",name:"Reine F.",time:"Il y a 1h",likes:18,text:"Excellente initiative ! Dans ma commune de Bohicon, on a essayé la même chose avec le lycée technique. Le plus difficile c'est de maintenir la motivation sur la durée. As-tu des astuces pour garder les jeunes engagés après les premiers mois ?"},{init:"IB",name:"Ibrahim M.",time:"Il y a 3h",likes:12,text:"Super article ! Je travaille sur un projet similaire à Kandi. Je pense que le digital est la clé — les jeunes sont beaucoup plus réceptifs quand on utilise WhatsApp et TikTok pour communiquer sur les ODD."},{init:"AT",name:"Adélaïde T.",time:"Il y a 5h",likes:7,text:"Merci pour ce partage. Est-ce que tu as eu du soutien de la mairie de Bohicon ? Dans mon cas à Parakou, le partenariat avec la commune a vraiment changé les choses pour obtenir des salles et du matériel."},{init:"FK",name:"Fidèle K.",time:"Il y a 8h",likes:4,text:"Très inspirant ! J'aimerais reproduire cela dans mon école à Cotonou. Tu serais disponible pour un échange direct pour partager tes expériences ?"}];return`
  <div class="fade-up">
    ...
    <div class="grid-3">
      <div style="display:flex;flex-direction:column;gap:14px;">
        ...
        <div class="card">
          <div class="card-body" style="padding:24px;">
            <div style="display:flex;gap:8px;margin-bottom:12px;flex-wrap:wrap;">
              ${t.tags.map(i=>`<span style="background:var(--green-soft);color:var(--green-mid);border-radius:20px;padding:3px 10px;font-size:11px;font-weight:600">${i}</span>`).join("")}
            </div>
            <h2 style="font-family:var(--font-display);font-size:22px;font-weight:700;color:var(--green-deep);margin-bottom:14px;line-height:1.3">${t.title}</h2>
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;padding-bottom:16px;border-bottom:1px solid var(--gray-100)">
              <div style="width:36px;height:36px;border-radius:50%;background:var(--green-soft);color:var(--green-mid);font-size:14px;font-weight:700;display:flex;align-items:center;justify-content:center">${t.author[0]}</div>
              <div><div style="font-size:13px;font-weight:600">${t.author}</div><div style="font-size:11px;color:var(--text-light)">${t.time} · Bohicon, Zou</div></div>
              <div style="margin-left:auto;display:flex;gap:8px">
                <span style="font-size:12px;color:var(--text-light)">💬 ${t.replies}</span>
                <span style="font-size:12px;color:var(--text-light)">❤️ ${t.likes}</span>
              </div>
            </div>
            ...
            <div style="display:flex;gap:8px;margin-top:16px;padding-top:16px;border-top:1px solid var(--gray-100)">
              <button class="btn btn-outline btn-sm">❤️ J'aime (${t.likes})</button>
              <button class="btn btn-outline btn-sm">📤 Partager</button>
        ...
        <!-- Réponses -->
        <div class="card">
          <div class="card-header"><span class="card-title">${t.replies} Réponses</span></div>
          <div class="card-body" style="padding:0">
            ${e.map((i,n)=>`
              <div style="padding:16px 20px;border-bottom:${n<e.length-1?"1px solid var(--gray-100)":"none"}">
                <div style="display:flex;align-items:flex-start;gap:10px;margin-bottom:10px">
                  <div style="width:32px;height:32px;border-radius:50%;background:var(--green-soft);color:var(--green-mid);font-size:12px;font-weight:600;display:flex;align-items:center;justify-content:center;flex-shrink:0">${i.init}</div>
                  <div style="flex:1">
                    <div style="display:flex;justify-content:space-between;align-items:baseline">
                      <span style="font-size:13px;font-weight:600">${i.name}</span>
                      <span style="font-size:11px;color:var(--text-light)">${i.time}</span>
                    </div>
                    <p style="font-size:13px;color:var(--text-sub);line-height:1.6;margin-top:6px">${i.text}</p>
                    <button style="background:none;border:none;font-size:11px;color:var(--text-light);cursor:pointer;margin-top:6px;padding:0">❤️ ${i.likes} · Répondre</button>
                  </div>
                </div>
              </div>`).join("")}
          </div>
          ...
          <div class="card-body">
            ${u.slice(1).map(i=>`
              <div onclick="navigate('forum-detail')" style="padding:10px 0;border-bottom:1px solid var(--gray-100);cursor:pointer">
                <div style="font-size:12.5px;font-weight:600;color:var(--text-main);margin-bottom:3px;line-height:1.4">${i.title}</div>
                <div style="font-size:11px;color:var(--text-light)">💬 ${i.replies} · ❤️ ${i.likes}</div>
              </div>`).join("")}
          </div>
        </div>
        <div class="card">
  ...
  </div>`}function V(){const t=g[0];return`
  <div class="fade-up">
    ...
    <div class="grid-3">
        ...
        <div class="card">
          <div class="card-body" style="padding:28px;">
            ...
            <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px">
              <span style="background:var(--green-soft);color:var(--green-mid);border-radius:4px;padding:3px 10px;font-size:12px;font-weight:600">${t.type}</span>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:20px">
              ${[{label:"Auteur",val:"INSAE Bénin"},{label:"Date",val:t.date},{label:"Pages",val:`${t.pages} pages`},{label:"Format",val:"PDF"},{label:"Langue",val:"Français"},{label:"Téléchargements",val:"1 247"}].map(e=>`
                <div style="padding:10px;background:var(--gray-100);border-radius:var(--radius-sm)">
                  <div style="font-size:10px;color:var(--text-light);font-weight:600;text-transform:uppercase;letter-spacing:.05em;margin-bottom:3px">${e.label}</div>
                  <div style="font-size:13px;font-weight:600;color:var(--text-main)">${e.val}</div>
                </div>`).join("")}
            </div>
            <div style="display:flex;flex-direction:column;gap:8px;">
      ...
      <div style="display:flex;flex-direction:column;gap:14px;">
        <div class="card">
          ...
          <div class="card-body">
            ${[{ch:"1",title:"Introduction et contexte",pages:"1–12"},{ch:"2",title:"Méthodologie et sources de données",pages:"13–24"},{ch:"3",title:"Résultats par ODD (1 à 6)",pages:"25–58"},{ch:"4",title:"Résultats par ODD (7 à 12)",pages:"59–92"},{ch:"5",title:"Résultats par ODD (13 à 17)",pages:"93–112"},{ch:"6",title:"Analyse territoriale par département",pages:"113–122"},{ch:"7",title:"Conclusions et recommandations",pages:"123–128"}].map(e=>`
              <div style="display:flex;align-items:center;justify-content:space-between;padding:9px 0;border-bottom:1px solid var(--gray-100);cursor:pointer"
                ...
                <div style="display:flex;gap:10px;align-items:center">
                  <span style="font-size:11px;color:var(--text-light);width:16px">${e.ch}.</span>
                  <span style="font-size:12.5px">${e.title}</span>
                </div>
                <span style="font-size:11px;color:var(--text-light)">${e.pages}</span>
              </div>`).join("")}
          </div>
        </div>
        <div class="card">
          ...
          <div class="card-body">
            ${g.slice(1,4).map(e=>`
              <div onclick="navigate('doc-detail')" style="display:flex;gap:10px;padding:10px 0;border-bottom:1px solid var(--gray-100);cursor:pointer">
                ...
                <div>
                  <div style="font-size:12.5px;font-weight:600;margin-bottom:2px">${e.title}</div>
                  <div style="font-size:11px;color:var(--text-light)">${e.type} · ${e.date}</div>
                </div>
              </div>`).join("")}
          </div>
        </div>
  ...
  </div>`}function K(){const t=b[0];return`
  <div class="fade-up">
    ...
    <div class="grid-3">
      <div style="display:flex;flex-direction:column;gap:14px;">
        <div style="background:linear-gradient(135deg,var(--navy),var(--green-deep));border-radius:var(--radius-xl);padding:32px 36px;color:white;">
          ...
          <div style="background:rgba(29,158,117,.25);border:1px solid rgba(29,158,117,.4);border-radius:20px;padding:4px 14px;font-size:11px;color:#6BDDAC;display:inline-block;margin-bottom:14px">📅 ${t.type}</div>
          <h2 style="font-family:var(--font-display);font-size:26px;font-weight:700;line-height:1.3;margin-bottom:10px">${t.title}</h2>
          <p style="font-size:14px;color:rgba(255,255,255,.65);line-height:1.7">${t.desc}</p>
        </div>

        <div class="card">
          <div class="card-header"><span class="card-title">Informations pratiques</span></div>
          <div class="card-body">
            ${[{icon:"📅",label:"Date",val:`${t.day} ${t.month} 2025`},{icon:"⏰",label:"Horaire",val:"8h30 – 17h00"},{icon:"📍",label:"Lieu",val:t.location},{icon:"👥",label:"Capacité",val:"500 participants"},{icon:"🎟️",label:"Accès",val:"Gratuit sur inscription"},{icon:"🌐",label:"Langue",val:"Français"}].map(e=>`
              <div style="display:flex;gap:12px;align-items:center;padding:10px 0;border-bottom:1px solid var(--gray-100)">
                ...
                <div>
                  <div style="font-size:11px;color:var(--text-light);font-weight:600">${e.label}</div>
                  <div style="font-size:13px;font-weight:500">${e.val}</div>
                </div>
              </div>`).join("")}
          </div>
        </div>
        ...
        <div class="card">
          ...
          <div class="card-body">
            ${[{time:"8h30",act:"Accueil et enregistrement"},{time:"9h00",act:"Cérémonie d'ouverture officielle"},{time:"9h45",act:"Présentation du rapport ODD Bénin 2024"},{time:"11h00",act:"Panels thématiques (4 salles en parallèle)"},{time:"12h30",act:"Déjeuner & networking"},{time:"14h00",act:"Ateliers de co-construction solutions"},{time:"16h00",act:"Restitution et recommandations"},{time:"17h00",act:"Clôture et cocktail"}].map((e,i)=>`
              <div style="display:flex;gap:12px;padding:8px 0;border-bottom:${i<7?"1px solid var(--gray-100)":"none"}">
                <span style="font-size:11px;font-weight:600;color:var(--green-mid);width:42px;flex-shrink:0;margin-top:2px">${e.time}</span>
                <span style="font-size:13px;color:var(--text-main)">${e.act}</span>
              </div>`).join("")}
          </div>
        </div>
      </div>
      ...
      <div style="display:flex;flex-direction:column;gap:14px;">
        ...
        <div class="card">
          ...
          <div class="card-body">
            ${[{init:"MK",name:"Ministère du Plan",role:"Représentant du gouvernement"},{init:"UN",name:"PNUD Bénin",role:"Partenaire technique"},{init:"UA",name:"Université d'Abomey",role:"Analyse des données"},{init:"OB",name:"ONG Bénin Vert",role:"Société civile"}].map(e=>`
              <div style="display:flex;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid var(--gray-100)">
                <div style="width:34px;height:34px;border-radius:10px;background:var(--green-soft);color:var(--green-mid);font-size:10px;font-weight:700;display:flex;align-items:center;justify-content:center">${e.init}</div>
                <div><div style="font-size:13px;font-weight:600">${e.name}</div><div style="font-size:11px;color:var(--text-light)">${e.role}</div></div>
              </div>`).join("")}
          </div>
        </div>
  ...
  </div>`}function U(){const t=d[0];return`
  <div class="fade-up">
    <button onclick="navigate('veille')" class="btn btn-ghost btn-sm" style="margin-bottom:16px;">← Retour à la veille</button>
    ...
    <div class="grid-3">
      <div style="display:flex;flex-direction:column;gap:14px;">
        <div class="card">
          <div class="card-body" style="padding:24px;">
            <div style="display:flex;gap:8px;margin-bottom:14px;align-items:center">
              ...
              <span style="background:${t.oddColor}22;color:${t.oddColor};border-radius:4px;padding:3px 10px;font-size:12px;font-weight:600">${t.oddLabel}</span>
              <span style="background:#FFE0DC;color:#D85A30;border-radius:4px;padding:3px 10px;font-size:12px;font-weight:600">🔴 Urgent</span>
              <span style="background:${t.statusColor}22;color:${t.statusColor};border-radius:4px;padding:3px 10px;font-size:12px;font-weight:600;margin-left:auto">${t.status}</span>
            </div>
            <h2 style="font-family:var(--font-display);font-size:22px;font-weight:700;color:var(--green-deep);margin-bottom:12px">${t.title}</h2>
            <p style="font-size:14px;color:var(--text-sub);line-height:1.7;margin-bottom:16px">
              ${t.desc} Les populations riveraines ont signalé une odeur chimique forte depuis 48h. Des poissons morts ont été observés sur 3km de cours d'eau. Les autorités locales ont été informées.
            </p>

            <div style="background:var(--gray-100);border-radius:var(--radius-md);padding:14px;margin-bottom:16px">
              ${[{icon:"📍",label:"Localisation",val:t.location},{icon:"⏱",label:"Signalé",val:t.time},{icon:"👤",label:"Signalé par",val:"Citoyen anonyme"},{icon:"🔁",label:"Statut",val:t.status}].map(e=>`
                <div style="display:flex;gap:10px;padding:6px 0">
                  <span>${e.icon}</span>
                  <span style="font-size:12px;color:var(--text-light);width:90px">${e.label}</span>
                  <span style="font-size:12px;font-weight:600">${e.val}</span>
                </div>`).join("")}
            </div>
        ...
        <div class="card">
          ...
          <div class="card-body">
            ${[{time:"Il y a 2h",actor:"Citoyen",text:"Signalement soumis avec photos",color:"#D85A30",done:!0},{time:"Il y a 1h",actor:"Modérateur",text:"Signalement validé et publié",color:"#E8A000",done:!0},{time:"En cours",actor:"Mairie Adjohoun",text:"Prise en charge par les autorités locales",color:"#185FA5",done:!1},{time:"À venir",actor:"Ministère Environnement",text:"Rapport d'inspection attendu",color:"#888",done:!1}].map((e,i)=>`
              <div style="display:flex;gap:12px;padding:10px 0;${i<3?"border-bottom:1px solid var(--gray-100)":""}">
                <div style="display:flex;flex-direction:column;align-items:center;gap:0">
                  <div style="width:12px;height:12px;border-radius:50%;background:${e.done?e.color:"var(--gray-200)"};flex-shrink:0;margin-top:4px"></div>
                  ${i<3?'<div style="width:2px;flex:1;background:var(--gray-200);margin:4px 0"></div>':""}
                </div>
                <div style="flex:1">
                  <div style="font-size:11px;color:var(--text-light);margin-bottom:2px">${e.time} · ${e.actor}</div>
                  <div style="font-size:13px;color:${e.done?"var(--text-main)":"var(--text-light)"}">${e.text}</div>
                </div>
              </div>`).join("")}
          </div>
        </div>
            ...
            ${d.slice(1,4).map(e=>`
              <div onclick="navigate('signal-detail')" style="padding:10px 0;border-bottom:1px solid var(--gray-100);cursor:pointer">
                <div style="display:flex;align-items:center;gap:6px;margin-bottom:3px">
                  <div style="width:7px;height:7px;border-radius:50%;background:${e.severity==="high"?"#D85A30":e.severity==="medium"?"#E8A000":"#1D9E75"}"></div>
                  <span style="font-size:10px;color:${e.oddColor};font-weight:600">${e.oddLabel}</span>
                </div>
                <div style="font-size:12.5px;font-weight:600;margin-bottom:2px">${e.title}</div>
                <div style="font-size:11px;color:var(--text-light)">📍 ${e.location} · ${e.time}</div>
              </div>`).join("")}
          </div>
        </div>
        <div class="card" style="background:var(--green-pale);border-color:var(--green-soft)">
        ...
        <div class="card-body" style="padding:28px;display:flex;flex-direction:column;gap:22px;">
          ...
          <div>
            ...
            <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px">
              ${c.slice(0,9).map(t=>`
                <button onclick="this.style.background='${t.color}22';this.style.borderColor='${t.color}';this.style.color='${t.color}'"
                  ...
                  ODD ${t.n} · ${t.label.split(" ").slice(0,2).join(" ")}
                </button>`).join("")}
            </div>
          </div>
  ...
  </div>`}window.currentPage="home";const x={home:{title:"Accueil",sub:"Bienvenue sur ODD Connect Bénin",fn:w},dashboard:{title:"Dashboard ODD",sub:"Suivi des 17 indicateurs · Bénin 2024",fn:k},plaidoyer:{title:"Plaidoyer citoyen",sub:"Campagnes actives et pétitions nationales",fn:$},veille:{title:"Veille citoyenne",sub:"Signalements et alertes en temps réel",fn:z},carte:{title:"Cartographie",sub:"Projets ODD et initiatives par commune",fn:A},forum:{title:"Forum communautaire",sub:"Discussions et co-construction citoyenne",fn:C},bibliotheque:{title:"Bibliothèque",sub:"Rapports, études et guides pratiques",fn:E},calendrier:{title:"Calendrier",sub:"Agenda des événements ODD au Bénin",fn:j},profil:{title:"Mon profil",sub:"Compte, activité et paramètres",fn:P},auth:{title:"Connexion",sub:"Accède à ton espace ODD Connect",fn:F},notifications:{title:"Notifications",sub:"4 notifications non lues",fn:I},parametres:{title:"Paramètres",sub:"Compte, notifications et préférences",fn:M},admin:{title:"Administration",sub:"Modération et gestion de la plateforme",fn:q},"campagne-detail":{title:"Détail campagne",sub:"Eau pour tous au Nord-Bénin · ODD 6",fn:O},"nouvelle-campagne":{title:"Lancer une campagne",sub:"Mobilise des citoyens autour des ODD",fn:H},"nouveau-signalement":{title:"Nouveau signalement",sub:"Signale un problème dans ta commune",fn:B},"signal-detail":{title:"Détail signalement",sub:"Déversement industriel · Adjohoun",fn:U},"odd-detail":{title:"ODD 6 · Eau propre",sub:"Indicateurs, objectifs et actions au Bénin",fn:S},"forum-detail":{title:"Discussion",sub:"Impliquer les jeunes dans le suivi des ODD",fn:G},"doc-detail":{title:"Rapport ODD Bénin 2024",sub:"INSAE · 128 pages · Mars 2024",fn:V},"event-detail":{title:"Forum National ODD 2025",sub:"15 Juin · Cotonou · 347 inscrits",fn:K},recherche:{title:"Recherche",sub:"Rechercher sur toute la plateforme",fn:J},partenaires:{title:"Réseau partenaires",sub:"78 organisations engagées pour les ODD",fn:N},apropos:{title:"À propos",sub:"Notre mission, équipe et parcours",fn:R},404:{title:"Page introuvable",sub:"Cette page n'existe pas",fn:T}};window.navigate=function(t){x[t]||(t="404"),window.currentPage=t;const e=x[t];document.getElementById("app").innerHTML=f(e.title,e.sub,e.fn()),window.scrollTo(0,0),setTimeout(()=>W(t),60)};function W(t){t==="dashboard"&&X(),t==="veille"&&Y(),t==="carte"&&Q()}const l={};function p(t){l[t]&&(l[t].destroy(),delete l[t])}function X(){p("lineChart");const t=document.getElementById("lineChart");t&&(l.lineChart=new Chart(t,{type:"line",data:{labels:v,datasets:[{data:h,borderColor:"#1D9E75",backgroundColor:"rgba(29,158,117,.08)",tension:.4,fill:!0,pointRadius:3,pointBackgroundColor:"#1D9E75",borderWidth:2},{data:v.map((n,s)=>55+s),borderColor:"#185FA5",borderDash:[5,4],pointRadius:0,borderWidth:1.5,fill:!1}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{x:{grid:{color:"rgba(0,0,0,.04)"},ticks:{font:{size:10},color:"#9AB0A8"}},y:{grid:{color:"rgba(0,0,0,.04)"},ticks:{font:{size:10},color:"#9AB0A8",callback:n=>n+"%"},min:50,max:70}}}})),p("radarChart");const e=document.getElementById("radarChart");e&&(l.radarChart=new Chart(e,{type:"radar",data:{labels:["Personnes","Planète","Prospérité","Paix"],datasets:[{data:[68,54,61,72],borderColor:"#1D9E75",backgroundColor:"rgba(29,158,117,.15)",borderWidth:2,pointBackgroundColor:"#1D9E75",pointRadius:4}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{r:{min:0,max:100,ticks:{display:!1},pointLabels:{color:"#4A6960",font:{size:11}},grid:{color:"rgba(0,0,0,.06)"},angleLines:{color:"rgba(0,0,0,.06)"}}}}})),p("barChart");const i=document.getElementById("barChart");i&&(l.barChart=new Chart(i,{type:"bar",data:{labels:v,datasets:[{data:D,backgroundColor:"rgba(29,158,117,.7)",borderRadius:3},{data:[12,18,22,28,35,41,38,45,52,60,71,84].map(n=>n*8),backgroundColor:"rgba(24,95,165,.5)",borderRadius:3}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{x:{grid:{display:!1},ticks:{font:{size:10},color:"#9AB0A8"}},y:{grid:{color:"rgba(0,0,0,.04)"},ticks:{font:{size:10},color:"#9AB0A8"}}}}}))}function Y(){p("signalChart");const t=document.getElementById("signalChart");t&&(l.signalChart=new Chart(t,{type:"doughnut",data:{labels:["ODD 6","ODD 7","ODD 13","ODD 14","ODD 15"],datasets:[{data:[32,18,24,15,11],backgroundColor:["#26BDE2","#FCC30B","#3F7E44","#0A97D9","#56C02B"],borderWidth:0}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{position:"bottom",labels:{font:{size:11},padding:8,color:"#4A6960"}}}}}))}let r=null;function Q(){!document.getElementById("map-container")||!window.L||(r&&(r.remove(),r=null),r=L.map("map-container").setView([9.3,2.3],7),L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"© OpenStreetMap"}).addTo(r),[{lat:6.37,lng:2.42,t:"Cotonou",c:"#1D9E75",tx:"Projet eau potable"},{lat:6.5,lng:2.62,t:"Porto-Novo",c:"#1D9E75",tx:"Éducation rurale"},{lat:9.34,lng:2.65,t:"Parakou",c:"#D85A30",tx:"🚨 Pénurie eau"},{lat:10.3,lng:1.38,t:"Natitingou",c:"#E8A000",tx:"École sans électricité"},{lat:7.18,lng:1.99,t:"Abomey",c:"#185FA5",tx:"📅 Atelier ODD"},{lat:6.95,lng:2.22,t:"Abomey-Calavi",c:"#1D9E75",tx:"Reboisement"},{lat:6.78,lng:1.98,t:"Lokossa",c:"#E8A000",tx:"Assainissement"},{lat:11.12,lng:2.88,t:"Kandi",c:"#1D9E75",tx:"Énergie solaire"},{lat:9.75,lng:1.61,t:"Djougou",c:"#D85A30",tx:"🚨 Déforestation"}].forEach(e=>{L.marker([e.lat,e.lng],{icon:L.divIcon({html:`<div style="width:14px;height:14px;border-radius:50%;background:${e.c};border:2px solid white;box-shadow:0 2px 6px rgba(0,0,0,.3)"></div>`,className:"",iconSize:[14,14]})}).addTo(r).bindPopup(`<strong>${e.t}</strong><br><span style="font-size:12px;color:#666">${e.tx}</span>`)}))}navigate("home");
